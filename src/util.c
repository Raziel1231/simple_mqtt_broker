...
void sol_log(int level, const char *fmt, ...)
{
  ...
  // Open two handler, one for standard ouput and a second for the persistent
  // log file
  FILE *fp = stdout;
  if (!fp) {
    return;
...
/* 
 * Parse the interger part of a string, by effectively iterate through 
 * it and converting the numbers found
 */
...
/*
 * Append a string to another, the destinetion string must be NUL-terminated
 * and long enough to contain the resulting string, for the chunk part that
 * will be appended the function requie the length, the resulting string will
